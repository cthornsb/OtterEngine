...
void OTTKeyboard::enable(){
	setupCallbacks();
}

void OTTKeyboard::disable(){
	setupCallbacks(false);
}

void OTTKeyboard::enableStreamMode(){
	bStreamMode = true;
	reset();
}

void OTTKeyboard::disableStreamMode(){
	bStreamMode = false;
	reset();
}

bool OTTKeyboard::poll(const unsigned char &key){ 
	if(states[key]){
		states[key] = false;
		return true;
	}
...
void OTTKeyboard::keyDown(const int &key, const int& mods){
...
void OTTKeyboard::keyUp(const int &key, const int& mods){
	if (bStreamMode)
		return;
...
void OTTKeyboard::checkKeyMods(const int& mods) {
	// Check key modifier
...
unsigned char OTTKeyboard::convertKey(const int &key, bool bKeyDown){
...
void OTTKeyboard::handleKeys(GLFWwindow* window, int key, int scancode, int action, int mods){
...
void OTTKeyboard::setupCallbacks(bool bEnable/*=true*/) {
	if(bEnable){
		// Set keyboard callback
		glfwSetKeyCallback(parent, OTTKeyboard::handleKeys);
