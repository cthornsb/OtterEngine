...
ott::Keyboard& ott::Keyboard::Instance() {
	static Keyboard instance;
	return instance;
}

ott::Keyboard::Keyboard() : 
	nCount(0), 
	bStreamMode(false),
	bLeftShift(false),
	bLeftCtrl(false),
...
void ott::Keyboard::Enable(){
	this->SetupCallbacks();
}

void ott::Keyboard::Disable(){
	this->SetupCallbacks(false);
}

void ott::Keyboard::EnableStreamMode(){
	bStreamMode = true;
	this->Reset();
}

void ott::Keyboard::DisableStreamMode(){
	bStreamMode = false;
	this->Reset();
}

bool ott::Keyboard::Poll(const uint8_t &key){ 
	if(states[key]){
		states[key] = false;
		return true;
	}
...
void ott::Keyboard::KeyDown(const int32_t &key, const int32_t& mods){
	// Check for key modifiers (e.g. shift, ctrl, etc)
	this->CheckKeyMods(mods);
	...
	uint8_t ckey = this->ConvertKey(key, true);
	if (!ckey) // Key has no corresponding ascii character
		return;

	if (bStreamMode == false) {
		if (!states[ckey]) {
			states[ckey] = true;
			nCount++;
		}
...
void ott::Keyboard::KeyUp(const int32_t &key, const int32_t& mods){
	if (bStreamMode == true) {
		return;
	}
	...
	// Even if 'key' is an ascii character, alpha-numeric characters may not necessarily be in order
	// on every platform. So we need to explicitly convert the key ID to a corresponding character.
	uint8_t ckey = this->ConvertKey(key, false);
	if (!ckey) // Key has no corresponding ascii character
		return;
...
void ott::Keyboard::CheckKeyMods(const int32_t& mods) {
	// Check key modifier
...
uint8_t ott::Keyboard::ConvertKey(const int32_t &key, bool bKeyDown){
	uint8_t ckey = 0x0;
	switch (key) {
	case GLFW_KEY_0:
...
bool ott::Keyboard::Get(char& key){
...
void ott::Keyboard::HandleKeys(GLFWwindow* window, int32_t key, int32_t scancode, int32_t action, int32_t mods){
	...
	case GLFW_PRESS:
		...
		currentWindow->GetKeyboard()->KeyDown(key, mods);
		break;
	case GLFW_RELEASE:
		currentWindow->GetKeyboard()->KeyUp(key, mods);
		break;
	case GLFW_REPEAT:
...
void ott::Keyboard::SetupCallbacks(bool bEnable/*=true*/) {
	if(bEnable){
		// Set keyboard callback
		glfwSetKeyCallback(parent, ott::Keyboard::HandleKeys);

		// Set keyboard keys to behave as button toggles
		this->DisableStreamMode();

		// Disable key repeat
		glfwSetInputMode(parent, GLFW_STICKY_KEYS, GLFW_TRUE);
		glfwSetInputMode(parent, GLFW_STICKY_MOUSE_BUTTONS, GLFW_TRUE);	
	}
	else{
		glfwSetKeyCallback(parent, 0x0); // Unset keyboard callback
	}
}
