...
void ott::WavHeader::print() const {
...
bool ott::WavHeader::readHeader(std::ifstream& f) {
...
bool ott::WavHeader::writeHeader(std::ofstream& f) {
...
ott::WavFilePlayer::WavFilePlayer(const std::string& fname) :
	WavFilePlayer()
{
	load(fname);
}

ott::WavFilePlayer::~WavFilePlayer() {
	audio.close();
}

bool ott::WavFilePlayer::load(const std::string& fname) {
	...
	if (audio.good()) {
		...
		if (nBitsPerChannel == 8) { // Unsigned 8-bit integer
			...
			uint32_t nBytesToRead = 3072;
			while (nLengthRemaining > 0) {
				nBytesToRead = std::min(nLengthRemaining, (uint32_t)3072);
				audio.read((char*)readBuffer, nBytesToRead);
				for (int32_t i = 0; i < nBytesToRead / 3; i++) {
...
float ott::WavFilePlayer::userSample(const float& dt) {
	if (!bGood) {
		return 0.f;
	}
	for (uint16_t ch = 0; ch < nChannels; ch++) { // Over all channels
		fInterpolated[ch] = fSamples[0][ch] + fPhase * (fSamples[1][ch] - fSamples[0][ch]) / fSamplePeriod;
	}
	return (fCurrentValue = fInterpolated[0]);
}

void ott::WavFilePlayer::userPhaseRollover() {
...
ott::WavFileRecorder::~WavFileRecorder() {
	if (bRecording)
		stopRecording();
}

bool ott::WavFileRecorder::startRecording(const std::string fname) {
	if (bRecording)
		return false;
...
bool ott::WavFileRecorder::stopRecording() {
	if (!bRecording)
		return false;
...
bool ott::WavFileRecorder::addSample(void* ptr) {
	if (!bRecording)
		return false;
...
bool ott::WavFileRecorder::addSample(const SoundMixer& mixer) {
	if (!bRecording)
		return false;
	if (nBitsPerChannel == 8) { // Unsigned 8-bit integer
		// -1 -> 0
