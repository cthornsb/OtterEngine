#ifndef MATH_OTT_VECTOR_HPP
#define MATH_OTT_VECTOR_HPP
...
public:
	...
	uint16_t dimension; ///< dimension of vector
	...
	/** Vector fill constructor
	  */
	Vector(const uint16_t& size, const float& value) :
		Vector(size)
	{
	...
	/** Explicit element constructor
	  */
	Vector(const uint16_t& size, const float* values) :
		Vector(size)
	{
		for (uint16_t i = 0; i < dimension; i++) { // over all elements
			elements[i] = values[i];
		}
	}
...
protected:
	...
	std::ostream& __stream(std::ostream& out) const {
		out << "(" << elements[0];
		for (uint16_t i = 1; i < dimension; i++) {
			out << ", " << elements[i];
		}
		out << ")";
		return out;
...
extern const Vector3 kZeroVector; ///< Three dimensional zero-vector (0, 0, 0)
extern const Vector3 kUnitVectorX; ///< X-axis unit vector (1, 0, 0)
extern const Vector3 kUnitVectorY; ///< Y-axis unit vector (0, 1, 0)
extern const Vector3 kUnitVectorZ; ///< Z-axis unit vector (0, 0, 1)

} /* namespace constants */
} /* namespace ott */

#endif // #ifndef MATH_OTT_VECTOR_HPP
